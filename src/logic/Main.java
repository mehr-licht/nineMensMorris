...
//				System.out.println("Game over. Player "+((LocalGame)game).getCurrentTurnPlayer().getPlayerToken()+" Won");
	...
	//	GameClient gc = null;
		...
		if(userInput.compareTo("SERVER") == 0 || userInput.compareTo("S") == 0) {
	        ...
	        try {
	            ...
	            for (NetworkInterface netint : Collections.list(nets)) {
	                ...
	                for (InetAddress inetAddress : Collections.list(inetAddresses)) {
	                    userinterface.log.info(inetAddress.toString());
	                }
	            }
			...
			userinterface.log.error("Unknown command. Closing program.");
			System.exit(-1);
		}
		...
		if(gs == null) { // this is only a client trying to connect
			while(true) {
				try {
					...
					userinterface.log.info("No GameServer detected!");
					if(--numberTries == 0) {
						System.exit(-1);
					}
					Thread.sleep(6000);
					userinterface.log.info("Trying another connection.");
				}
			}
			...
			while(true) {
				...
				if(firstTry) {
					userinterface.log.info("Server initialized. Waiting for connection from GameClient of this computer");
					firstTry = false;
				}
				Thread.sleep(100);
			}
