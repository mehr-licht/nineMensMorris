...
import logic.IAPlayer;
import logic.LocalGame;
import logic.MinimaxIAPlayer;
import logic.Move;
import logic.NetworkGame;
import logic.Player;
import logic.RandomIAPlayer;
import logic.Token;
import delete.network.GameClient;
import delete.network.GameServer;
import delete.slidinglayout.SLAnimator;
import delete.slidinglayout.SLConfig;
import delete.slidinglayout.SLKeyframe;
import delete.slidinglayout.SLPanel;
import delete.slidinglayout.SLSide;
...
//								}
					...
					for(int i = 0; i < coords.length; i++) {
						...
						if(x >= c.x && y >= c.y && x < (c.x + 100) && y < (c.y + 100)) { // a btn was pressed
							...
							} else { // choosing type of delete.network
								if(game_type == UIResourcesLoader.NETWORK_GAME) {
